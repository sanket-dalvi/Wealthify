...
Create postgres user (superuser) with name and password as wealthify

```
CREATE USER wealthify WITH PASSWORD 'wealthify' SUPERUSER;
```
Create Database with name 'wealthify' and assign the above created user to the database with preferable CLI or GUI tool
...
```
grant all privileges on database wealthify to wealthify;
```
...
The Default port is 9090 and base-url is set to /wealthify (both can be changed in application.properties)

Go to the below URI to view Swagger-UI (API-docs)

```
http://localhost:9090/wealthify/swagger-ui.html
```
